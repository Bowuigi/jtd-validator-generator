...
testCase("simple ref rejects type mismatch", SIMPLE_REF, "foo", [{ path: [], message: "expected float32, got string", suggestions: [] }]);
testCase("simple ref rejects null", SIMPLE_REF, null, [{ path: [], message: "expected float32, got null", suggestions: [] }]);
...
testCase("nullable ref still rejects type mismatch", NULLABLE_REF, "foo", [{ path: [], message: "expected float32, got string", suggestions: [] }]);
...
testCase("ref to properties rejects with missing property and type error", REF_TO_PROPERTIES, { lat: "foo" }, [
...
testCase("ref in elements rejects with error at index", REF_IN_ELEMENTS, [1, "foo"], [{ path: [1], message: "expected float32, got string", suggestions: [] }]);
...
testCase("recursive list accepts single node with null next", RECURSIVE_LIST, { value: 1, next: null });
testCase("recursive list accepts two nodes", RECURSIVE_LIST, { value: 1, next: { value: 2, next: null } });
testCase("recursive list accepts three nodes", RECURSIVE_LIST, { value: 1, next: { value: 2, next: { value: 3, next: null } } });
testCase("recursive list rejects invalid at depth", RECURSIVE_LIST, { value: 1, next: { value: "bad" } }, [
